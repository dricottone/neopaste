...
def main(arguments):
	config=dict()
	positional=[]
	pattern=re.compile(r"(?:-(?:c|d|h|x|l|r|v|V)|--(?:center|delimiter|help|left|right|version))(?:=.*)?$")
		...
		if consuming is not None:
			...
			if option=="center":
				if attached_value is not None:
					message=(
						'unexpected value while parsing "center"'
						' (expected 0 values)'
					)
				...
				if attached_value is not None:
					config["delimiter"]=attached_value
					attached_value=None
					consuming,needing,wanting=None,0,0
				else:
					config["delimiter"]=None
					consuming,needing,wanting="delimiter",1,1
			elif option=="help":
				if attached_value is not None:
					message=(
				...
				if attached_value is not None:
					message=(
						'unexpected value while parsing "left"'
						' (expected 0 values)'
					)
					raise ValueError(message) from None
				config["left"]=True
				...
				if attached_value is not None:
					message=(
						'unexpected value while parsing "right"'
						' (expected 0 values)'
					)
					raise ValueError(message) from None
				config["right"]=True
